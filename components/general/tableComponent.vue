...
<template>
  <div v-if="source">
    <div v-if="source.data" class="rounded-lg overflow-x-auto text-xs">
      <table
        :class="[{ 'pe-none': spinner }]"
        class="hidden md:block custom-table dark:border-darkElBg text-black dark:text-darkText min-w-full bg-white dark:bg-darkElBg text-xs"
      >
        <div
          v-if="spinner"
        ...
        >
          <tr class="">
            <th
              ...
              :key="key"
              class="py-2 md:py-3 px-3 font-bold !text-[#9A9BA4] dark:text-darkText text-xs whitespace-nowrap"
            >
              {{ column.name }}
            </th>
          </tr>
        ...
        <tbody>
          ...
          <template v-else>
            <tr
              ...
              class="whitespace-nowrap md:whitespace-normal text-xs font-medium !rounded-lg"
            >
              <td
                ...
                class="py-2 md:py-5 px-3 border-gray-300 dark:border-gray-600"
              >
                <slot
                  :column="column"
                  :index="key"
      ...
      <div
        :class="{ 'pe-none': spinner }"
        class="block md:hidden text-[#757575] dark:text-darkText dark:bg-darkElBg text-xs"
      >
        <div
          v-if="spinner"
          class="position-absolute d-flex align-items-baseline h-100 w-100 top-0 bg-white bg-opacity-50"
              ...
              >
                ...
                <div>
                  <div
                    ...
                    :class="[column.tdClass, { 'position-relative': linked }]"
                    :data-column-name="column.name"
                    class="py-2 md:py-3 px-3 text-xs border-gray-300 dark:border-gray-600 text-left"
                  >
    ...
    <div v-else class="rounded-lg">
      <table
        :class="[
          { 'pe-none': spinner },
          { archiveTable: $route.name === 'ArchiveGroups' },
        ]"
        class="hidden md:block custom-table dark:border-darkElBg text-black dark:text-darkText min-w-full bg-white dark:bg-darkElBg text-xs overflow-x-auto"
      >
        <div
          v-if="spinner"
          class="position-absolute d-flex align-items-baseline h-100 w-100 top-0 bg-white bg-opacity-50"
          ...
          <template v-else>
            ...
            >
              <td
                ...
                :class="[
                  column.tdClass,
                  { 'position-relative': linked },
                  { 'rounded-l-lg': index === 0 },
                  { 'rounded-r-lg': columns.length === index + 1 },
                ]"
                :data-column-name="column.name"
                class="py-2 md:py-3 px-3 border-gray-300 dark:border-gray-600"
              >
                <slot
                  :column="column"
                  :index="key"
    ...
    <div
      ...
      class="block mt-4"
    >
      <NewPagination :pagesData="pagination" @navigate="navigateLink" />
    </div>
  </div>
</template>
